...
internal class OrderMonitorQuery(TechFoodContext dbContext, IImageUrlResolver imageUrlResolver) : IReadOnlyQuery<GetOrderMonitorResult>
{
    ...
    public async Task<IEnumerable<GetOrderMonitorResult>> GetAllAsync()
    {
       ...
       .Select(order => new
       {
        ...
        var result = orders.Select(order => new GetOrderMonitorResult
        {
            OrderId = order.Id,
            Status = order.Status,
            Number = order.Number,
